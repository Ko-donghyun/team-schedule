...
router.post('/exit', (req, res, next) => {
  console.log('그룹 나가기 미들웨어 시작');

  const userId = req.body.user_id;
  const groupId = req.body.group_id;

  console.log('유효성 검사 시작');
  groupValidation.exitGroup(userId, groupId).then(() => {
    console.log('유효성 검사 완료');
    console.log('유저 그룹 조회 시작');
    ...
    if (!userGroup.approval) {
      throw helper.makePredictableError(200, '수락되지 않은 그룹 입니다.');
    }
    ...
    res.json({
      success: 1,
      message: '그룹 나가기 성공했습니다',
    });
  }).catch((err) => {
    if (err.statusCode !== 200) {
      console.log('그룹 나가기 중 에러: %s', err.stack);
    }
